---
title: Configure Genesys Cloud for two way messaging using Open Messaging Platform and Sinch Messages API
author: kitt.phi
indextype: blueprint
icon: blueprint
image: flowchart.png
category: 3
summary: |
  This Genesys Cloud Developer Blueprint provides instructions to configure a middleware to allow a Genesys Cloud Agent to send and receive messages from a Sinch Virtual Number (LVN), using the Open Messaging Platform and Sinch Messages API.
---

![Flowchart for Open Messaging to Sinch Messages API](flowchart.png "Flowchart for Open Messaging to Sinch Messages API")
...
- **Sinch Messages API platform** - The Messages API enables you to send messages to customers via their preferred channels (currently Facebook Messenger, WhatsApp, Viber, and SMS/MMS) using a single API.
...
- **Sinch NodeJS Server SDK** - This SDK is used to create an instance of the Sinch Messages API by providing Sinch Account and Application Credentials.
...
- Experience with Sinch Messages API

### Genesys Cloud and Sinch account

- A Genesys Cloud license. For more information, see [Genesys Cloud pricing](https://www.genesys.com/pricing "Opens the Genesys Cloud pricing page") on the Genesys website.
- **Recommended** The Master Admin role in Genesys Cloud. For more information, see [Roles and permissions overview](https://help.mypurecloud.com/?p=24360 "Opens the Roles and permissions overview article") in the Genesys Cloud Resource Center.
- Sinch Account. To sign up for a new account, visit [Signup for Vonagage Account](https://dashboard.nexmo.com/sign-up?icid=tryitfree_homepage_nexmodashbdfreetrialsignup_tile&utm_campaign=Sinchseo).
...
- [Sinch Setup](#Sinch-setup "Sinch Setup")
- [Genesys Setup](#genesys-setup "Genesys Setup")
- [Start Express Server](#start-express-server "Start Express Server")

### Download the repository containing the project files

1. Go to the GitHub [https://github.com/GenesysCloudBlueprints/Sinch-sms-api-blueprint/](https://github.com/GenesysCloudBlueprints/Sinch-sms-api-blueprint/ "Opens the genesys open messaging GitHub repository") and clone the repository to your machine.

   **Note** If you don't have access to it, you can contact the slack channel [#ask-cse](https://Sinch.slack.com/archives/CNW647A0Y) for inquiry.
...
1. The solution requires a Sinch Account and Sinch Virtual Number. If you don't already have an account, you'll need to [Signup for Sinch Account](https://dashboard.nexmo.com/sign-up?icid=tryitfree_homepage_nexmodashbdfreetrialsignup_tile&utm_campaign=Sinchseo). If you already have a Sinch Account then you can proceed to the [Sinch Dashboard](https://dashboard.nexmo.com).

2. Purchase a Sinch Virutal Number here at the [Sinch Dashboard](https://dashboard.nexmo.com/your-numbers). Save that Virtual Number to the `VIRTUAL_NUMBER` variable at the`.env` file.

3. Configure Settings for Messages API here at the [Sinch Dashboard](https://dashboard.nexmo.com/settings). At SMS Settings, enable Messages API, then Save Settings. Notice the `API key` and `API secret`. Save that to the `Sinch_API_KEY` and `Sinch_API_SECRET` variable at the`.env` file.

4. Create a Sinch Messages API Application here at the [Sinch Dashboard](https://dashboard.nexmo.com/applications). Enable Messages and set the webhooks as below, while replacing `NGROK-URL` with the URL you receieved from Ngrok.
...
5.  Configure Architect Flow for Inbound Message. Navigate to Genesys Cloud > Admin > Architect. Select `Flows: Inbound Message` from the dropdown list. Click `+ Add`, then you can name it `SinchMessage`.

6.  Configure Routing. Navigate to Genesys Cloud > Admin > Routing > Message Routing. Add via `+` icon. You can name Inbound Message Flows `SinchMessage` as you did with the Architect Flow above and Inbound Address to `SinchMessaging`. You can name the Description `Inbound Sinch OpenMessaging`. Set the Intial State to `Transfer to ACD`. You can save the name as `Transfer to ACD` and Queue to `Sinch`. Make sure to Add ACD Skill. Save the settings.
